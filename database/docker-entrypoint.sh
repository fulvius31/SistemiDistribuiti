...
if [[ -z $ADMIN_PASSWORD ]]; then
	ADMIN_PASSWORD="SistemiDistribuiti2017"
...
EOF
		...
		if [ "$MYSQL_DATABASE" ]; then
			...
			#Little for create table in database for sistemidistribuiti 
			echo "USE sistemidistribuiti; CREATE TABLE log(id VARCHAR(30), timestamp VARCHAR(60), msg VARCHAR(130));" | "${mysql[@]}"
...
exec "$@" & exec "mysqld" & exec "/root/leaderelection.sh"
